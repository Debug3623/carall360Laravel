...
class PrescriptionController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $oValidator = Validator::make($oInput,[
            ...
            'type'          => 'required|in:tablet,capsule,syrup,drops,inhaler,injection,topical,patch,ointment,spray,sach,shampoo,vaccine,guaze,anticeptic,balm,vial,cream,lotion',
    ...
    public function uploadPrescription(Request $request)
    {
        ...
        if($request->hasFile('record')){
            ...
            foreach ($oFiles as $oFile) {
                $imageName = $oFile->getClientOriginalName();
                $imageNameDate = date('U').$i.'_'.$imageName;
                $i++;
                $extension = $oFile->extension();
                if($extension == "png" || $extension == "jpg" || $extension == "jpeg"){
                    
                    $img = Image::make($oFile);
                    $imageSize = filesize($oFile)/1000;
                    if($imageSize >= "400"){
                    ...
                    $mPutFile = 'prescription/'.md5($oInput['patient_id']).'/'.md5($imageNameDate).'.'.$extension;
                    $image = Storage::disk()->put($mPutFile,$img->encode());

                }else{
                    $mPutFile = Storage::disk()->putFile('prescription/'.md5($oInput['patient_id']), $oFile);
                }
    ...
    public function showUploadedPrescription(Request $request, $url)
    {
        ...
        $file = Storage::disk()->get($sFileUrl);
