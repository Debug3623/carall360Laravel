        ...
        if(!empty(request('username')) && !empty(request('password')))
        {
            ...
            if($bIsOtpBasedLogin)
            {
                ...
                if($sLoginSource == "BOP"){          // Back Staff OTP
                    ...
                    $oUser = User::select('id', 'password', 'password_store')->where('register_number', request('register_number'))->where('doctor_id',$iDoctorId)->whereNotNull('patient_id')->first();
                }else{                              // Patient OTP
                    if(!empty($iDoctorId)){
                        $oUser = User::select('id', 'password', 'password_store')->where('register_number', request('register_number'))->where('doctor_id',$iDoctorId)->whereNotNull('patient_id')->first();
                    }else{
                        $oUser = User::select('id', 'password', 'password_store')->where('register_number', request('register_number'))->first();
                    }      
                }
                if(!empty($iDoctorId))
